...
const getCheckout = async (req, res, next) => {
  try {
    ...
    for (const item of cart.items) {
      ...
      const offerResult = await calculateItemPrice(
        product,
        variant,
        item.categoryId?._id
      );
...
const placeOrder = async (req, res, next) => {
  try {
    ...
    if (paymentMethod === "RAZORPAY") {
      ...
      if (existingOrder) {
        return res.status(HTTP_STATUS.BAD_REQUEST).json({
          success: false,
          message:
            "You already have a pending  payment. Retry from Orders page.",
        });
      }
    }
    ...
    for (let i = 0; i < cart.items.length; i++) {
      ...
      if (appliedCoupon) {
        if (i === cart.items.length - 1) {
          ...
          itemCouponShare = Math.floor(
            (itemTotal / (recalculatedTotal || 1)) *
    ...
    const finalAmount =
      orderItems.reduce((sum, i) => sum + i.finalItemAmount, 0) +
    ...
    if (!finalAmount || isNaN(finalAmount) || finalAmount <= 0) {
      throw new AppError(
        `Invalid payment amount: ${finalAmount}`,
        HTTP_STATUS.BAD_REQUEST
      );
    }
    ...
    const order = await Order.create({
      ...
      coupons: appliedCoupon
        ? {
            ...
            minOrderPrice: appliedCoupon.minOrderPrice,
...
const verifyRazorpayPayment = async (req, res, next) => {
  try {
    ...
    if (
      !order.payment?.razorpay ||
      order.payment.razorpay.orderId !== razorpay_order_id
    ) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({
        success: false,
...
const createRetryRazorpayOrder = async (req, res, next) => {
  try {
    ...
    if (order.payment.status !== "FAILED") {
      return res
        .status(HTTP_STATUS.BAD_REQUEST)
        .json({ success: false, message: "Payment retry not allowed" });
    }
